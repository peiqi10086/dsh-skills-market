window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"action.delete": "删除",
			"action.deleteHint": "确认删除该 skill？此操作不可恢复",
		...
		const en = {
			...
			"action.deleteHint": "Delete this skill? This cannot be undone",
			"action.deleteConfirm": "Confirm delete",
			"action.cancel": "Cancel",
			"toggle.enabled": "Enabled",
			"toggle.disabled": "Disabled",
			"toggle.enable": "Click to enable",
			"toggle.disable": "Click to disable",
		...
		var SkillsStore = class {
			...
			* 合并为一次真实写入，避免抖动。
			*/
			queueInvocationToggle(rowKey, body) {
				...
				this.toggleTimers[rowKey] = setTimeout(() => {
					...
					this.runRowAction(rowKey, "set-invocation", body);
				}, 400);
			}
			/** 乐观更新：把新开关状态直接补丁进本地清单缓存（绕过 watcher 去抖延迟）。 */
			patchInvocation(body) {
				const patch = {};
				...
				const groupData = {};
				for (const [key, group] of Object.entries(this.state.groupData)) groupData[key] = group === void 0 ? group : {
					...group,
					skills: group.skills.map((s) => s.dir === body["name"] && s.level === body["level"] ? {
						...s,
						...patch
					} : s)
				};
		...
		*/
		function SkillsEntry({ wide, store, t, onOpen }) {
		...
		function useScrollTopOnPage(...deps) {
		...
		function SkillRow({ item, store, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: `dshs-row${item.manageable ? "" : " readonly"}${enabled ? "" : " dimmed"}`,
				children: [
					...
					item.manageable ? confirming ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dshs-actions",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dshs-confirm-hint",
								children: t("action.deleteHint")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								...
								disabled: isBusy,
								onClick: () => {
									store.runRowAction(rowKey, "uninstall", {
										name: item.dir,
										level: item.level
									});
								},
								children: isBusy ? t("action.working") : t("action.deleteConfirm")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								onClick: () => {
									store.queueInvocationToggle(rowKey, {
										name: item.dir,
										level: item.level,
								...
								children: isBusy ? t("action.working") : `${enabled ? t("toggle.enabled") : t("toggle.disabled")} ${enabled ? "✓" : "✕"}`
							}),
							item.level === "project" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dshs-btn ghost",
								disabled: isBusy,
								onClick: () => {
		...
		*/
		...
		function SkillsOverlay(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PanelBody, { ...props })]
			})] });
		}
		//#endregion
...
`;
		...
		*/
		...
		function apply(ctx) {
			...
			const getBetterSidebar = () => ctx.get("betterSidebar");
			/** 向 better-sidebar 注册「Skills 管理」tab（幂等；ctx.effect 随 fiber 撤销）。 */
			function ensureTabRegistered(bs) {
				...
				try {
					ctx.effect(() => bs.registerTab({
						...
						icon: (size) => (0, react.createElement)(_deepseek_ai_dsh_client_ui_primitives.IconSparkle16, { size }),
						order: 60,
						single: true,
						component: () => (0, react.createElement)(PanelBody, {
							store,
							t: sharedT
						})
			...
			/** 在状态树里查找本 tab 的位置。 */
			function bsTabLocation(bs) {
				if (typeof bs.getSnapshot !== "function") return "closed";
				const snapshot = bs.getSnapshot();
				const state = snapshot === void 0 ? void 0 : snapshot.state;
				if (state === void 0 || state === null) return "closed";
				const hasTab = (node) => {
			...
			*/
			function clickPanelToggle(rectX, rectWidth) {
				const buttons = document.querySelectorAll("button");
				for (const button of buttons) if (button.querySelector(`rect[x="${rectX}"][width="${rectWidth}"]`) !== null) {
					button.click();
					return true;
				}
				return false;
			}
			const RIGHT_TOGGLE = ["10.5", "2.75"];
			const BOTTOM_TOGGLE = ["3.25", "9.5"];
			/** 入口点击：有 better-sidebar → 切换本 tab 所在工作台的开/关；否则默认浮窗。 */
			function openManager() {
				const bs = getBetterSidebar();
				if (bs !== void 0 && typeof bs.registerTab === "function") {
			...
			if (early !== void 0 && typeof early.registerTab === "function") ensureTabRegistered(early);
			ctx.on("internal/service", ((name, value) => {
				if (name !== "betterSidebar" || value === void 0 || value === null) return;
				const bs = value;
			...
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				sharedT = props.t;
				return (0, react.createElement)(SkillsEntry, props);
			}));
			ctx.slots.inject("shell.overlay", () => ctx.slots.register({
				...
				sharedT = props.t;
				return (0, react.createElement)(SkillsOverlay, props);
			}));
		}
		//#endregion
		exports.apply = apply;
