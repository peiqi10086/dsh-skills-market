...
- **已安装管理**：用户级（所有工作区共享）、每个工作区一个页签（打开面板自动定位当前工作区）、内置只读展示；组内筛选（名称/简介）；卸载（两步确认）、项目级升用户级；**调用面启停**（`disable-model-invocation` / `user-invocable` 两个 frontmatter 开关，逐 skill 控制 AI 调用与 /name 用户调用）；分页（页码窗口 + 跳页 + 10/20/30/50/100 条/页）
